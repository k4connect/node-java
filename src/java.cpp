...
void EIO_AfterCallJs(uv_work_t* req) {
  ...
  if(callData->done != 0) {
    printf("*** ERROR: EIO_AfterCallJs called twice for same dynamicProxyData!!!!");
    return;
  }
  ...
  v8::Local<v8::Value> fnObj = dynamicProxyDataFunctions->Get(Nan::New<v8::String>(callData->methodName.c_str()).ToLocalChecked());
  ...
  if (tryCatch.HasCaught()) {
    ...
    if (stackTrace.length() > 0) {
      callData->throwableMessage = std::string(*stackTrace);
    } else {
      v8::String::Utf8Value exception(tryCatch.Exception());
      callData->throwableMessage = std::string(*exception);
    }
    tryCatch.Reset();
    callData->done = DYNAMIC_PROXY_JS_ERROR;
    return;
